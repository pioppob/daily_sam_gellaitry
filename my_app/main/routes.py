...
from flask import render_template, redirect, url_for, send_file, jsonify, make_response
...
def shuffle():
    ...
    return render_template('shuffle.html', title='Shuffle Tracks', tracks=tracks)
...
def add_json_tracks():
    ...
    Unfortunately soundcloud has stopped processing API application requests so everything from here on
    is reverse engineered using a web-scraper and tracing network IO in chrome devtools.
    """
    ...
    for row in json_content:
        ...
        request_url = row['audio_request_url'] + f'?client_id={Config.CLIENT_ID}'
...
def play_track(track_id):
    """
    Purpose: 
    This function is invoked for every track displayed on the page. It is imbedded in the
    HTML audio tag's src property and should return the audio to make the audio tag playable.

    Info on the track.audio property:
    Soundcloud makes an AJAX call to the url in track.audio which returns a response containing 
    the .mpeg audio url. The response expires after one call and is programmatically generated on
    every request so no two responses are the same. This makes it a pain to work with.

    Goal:
    Simulate the soundcloud AJAX call for every track on the page, 
    parse the .mpeg URL from the response and return the audio in a format that
    an HTML <audio> player can understand.
    """ 
   ...
   # My issue is here: If I hardcode the mpeg_audio_url into the <audio src> HTML property, it works!
   # However, returning the mpeg_audio_url as we do below does not work. My head hurts.
...
def track_of_the_day():
    ...
    return render_template('daily_sam.html', title='Track of the day', track=track)
...
def add_local_tracks():
    ...
    # os.chdir('/Users/Pioppo/Desktop/Projects/hotTakes/my_bp/static/tracks')
